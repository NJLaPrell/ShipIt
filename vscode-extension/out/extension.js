...
function inferIntentFromActiveEditor() {
...
async function resolveIntentId(commandId) {
...
async function runShipitCommand(commandId) {
    ...
    try {
        ...
        vscode.window.showErrorMessage(`ShipIt: Failed to open chat — ${err instanceof Error ? err.message : String(err)}`);
    }
}
