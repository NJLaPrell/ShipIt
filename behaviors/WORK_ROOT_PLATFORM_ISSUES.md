...
**When to use:** Any time you are working a platform issue from `./testing/ISSUES.md`.

## Scope and Safety

- This behavior applies to the root platform repo (not downstream project repos).
- Read issues from `./testing/ISSUES.md` only.
- After resolving an issue, update it in `./testing/ISSUES.md` and then move it to `./testing/ISSUES_HISTORY.md`.
- If the issue cannot be resolved without changing scope or requirements, stop and report.
- If `./testing/ISSUES.md` or `./testing/ISSUES_HISTORY.md` is missing, stop and report.
...
**Actions:**
1. Read the issue entry in `./testing/ISSUES.md`.
...
3. If the issue description is outdated or incorrect:
   ...
   - If the issue is no longer valid, mark it resolved and move it to `./testing/ISSUES_HISTORY.md`.
...
**Actions:**
1. Update the issue in `./testing/ISSUES.md` with:
   - Resolution summary
   - Validation performed
   - Date resolved
2. Move the issue entry to `./testing/ISSUES_HISTORY.md`.
