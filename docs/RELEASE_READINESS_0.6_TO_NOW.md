...
| -------------------- | --------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| **Install**          | Package name is now **@njlaprell/shipit** (unscoped `shipit` is taken on npm).                                        | User must run `npm install -g @njlaprell/shipit`. If the package isn’t published yet, they get 404 until first publish. |
...
1. **Install command wrong in one place**  
   README “Installation > For App Developers” still said `npm install -g shipit`. Fixed to `@njlaprell/shipit`.

2. **404 until first publish**  
   If we haven’t published `@njlaprell/shipit` yet, `npm install -g @njlaprell/shipit` fails. Mitigation: TROUBLESHOOTING explains install; after first publish this goes away. Optional: one line in README “(package available after first release)” or “from source” in CONTRIBUTING.
...
- [x] Install commands use `@njlaprell/shipit` everywhere in README/docs.
- [x] Deprecated CTA updated; contributor vs app-developer paths clear.
- [ ] Publish `@njlaprell/shipit` so `npm install -g @njlaprell/shipit` works.
- [ ] Bump version (e.g. 0.7.0), CHANGELOG, tag, GitHub release (see RELEASE_CHECKLIST.md).

## Verifying first-time user path

1. **Install** (after publish): `npm install -g @njlaprell/shipit` → success.
