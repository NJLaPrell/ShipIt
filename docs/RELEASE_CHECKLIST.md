...
- [ ] **First-time only:** npm name — `shipit` is taken; use a scoped name (e.g. `@nlaprell/shipit`). Set in `package.json` and update README install instructions. See [PUBLISHING.md](./PUBLISHING.md).
...
- [ ] If published to npm: verify `npm install -g @nlaprell/shipit` and run `shipit --help`, `shipit check`
