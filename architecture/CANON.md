...
**Full map:** See [DIRECTORY_STRUCTURE.md](../DIRECTORY_STRUCTURE.md) for a quick reference.

| Directory          | Purpose                                                                         | Owner              | Modification Rules                                     |
| ------------------ | ------------------------------------------------------------------------------- | ------------------ | ------------------------------------------------------ |
| `/intent/`         | Planned work (features, bugs, tech-debt)                                        | PM, Steward        | Anyone can propose; Steward approves                   |
| `/architecture/`   | System boundaries, invariants, schemas, ADRs                                    | Architect, Steward | Requires ADR for changes                               |
| `/workflow-state/` | Current execution state (phases, status, assumptions)                           | All agents         | Append-only during execution                           |
| `/do-not-repeat/`  | Failed approaches (abandoned designs, bad patterns, rejected libs)              | All agents         | Append-only; never delete                              |
| `/src/`            | Production source code                                                          | Implementer        | Must follow approved plan                              |
| `/tests/`          | Test code, fixtures, test plans, run logs                                       | QA, Implementer    | Tests before implementation                            |
| `/artifacts/`      | Generated files (SYSTEM_STATE.md, dependencies.md, confidence-calibration.json) | Scripts            | Regenerated by generate-\* and verify scripts          |
| `/release/`        | Generated release plan (ordered intents by release target)                      | Scripts            | Regenerated by generate-release-plan.sh                |
| `/roadmap/`        | Planning views (now, next, later triage)                                        | Scripts            | Regenerated by generate-roadmap.sh                     |
| `/scripts/`        | Shell scripts for workflow, generation, validation, deploy                      | All                | Add with approval; follow lib/ for shared code         |
| `/security/`       | Audit allowlist, security config                                                | Security, Steward  | Append-only for allowlist                              |
| `/behaviors/`      | Procedures and policies (release, issue tracking, platform work)                | All                | Document how we work                                   |
| `/reports/`        | Generated reports (mutation, coverage, etc.)                                    | Scripts            | Regenerated; often gitignored                          |
| `/golden-data/`    | Replay validation test data for regression testing                              | QA                 | Append-only; capture known-good inputs/outputs         |
| `/projects/`       | Initialized ShipIt projects (from /init-project)                                | Scripts            | Contents gitignored; .gitkeep only committed           |
| `/drift/`          | Entropy baselines and metrics                                                   | Scripts            | Regenerated by drift-check.sh                          |
